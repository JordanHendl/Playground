...
#include "raw_data/diffuse_image.hpp"
#include "raw_data/specular_image.hpp"
...
struct DeferredTestData {
  ...
  luna::gfx::MultiBuffered<luna::gfx::CommandList> cmd;
...
auto create_second_subpass() -> gfx::Subpass {
  ...
  subpass.attachments.push_back({"GFinal", data->window.image_views(), cClearColors});
...
auto initialize() -> void {
  ...
  data->cmd = std::move(gfx::MultiBuffered<gfx::CommandList>(cGPU));
...
auto draw_loop() -> void {
  auto& cmd = data->cmd;
  static auto camera = data->camera_info.get_mapped_container();
  static auto transform = data->transforms.get_mapped_container();
...
extern "C" auto initialize() -> void {
  std::cout << "Node start!" << std::endl;
  data = std::make_unique<DeferredTestData>();
  luna::initialize();
}

extern "C" auto update() -> bool {
  std::cout << "Node tick!" << std::endl;
  luna::draw_loop();
  return running;
}

extern "C" auto shutdown() -> void {
