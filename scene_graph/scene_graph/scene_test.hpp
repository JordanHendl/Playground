...
inline auto test_scene_graph() -> void {
  ...
  scene.add_child("text a", {"Text", "ComicSans", {{0.2f, 0.5f, 0.7f}}});
  auto node = scene.add_child("text b", {"Text", "SansSerif", {{0.9f, 2.f, 10.f}}});
  auto tmp = node;
  for(auto i = 0u; i < cNumNodes; ++i) {
    tmp = scene.add_child(std::to_string(i), {"Image", "MonaLisa"}, tmp);
  }
